...
def index():
    ...
    global ticker_for_auto_update
    ticker_for_auto_update = ticker
...
@app.route('/get_close')
def update_today_close():
    while True:
        stock = yf.Ticker(ticker_for_auto_update)
        data = stock.history(period='1d')
        if not data.empty:
            close_c = data['Close'].iloc[-1]
            close_update = str(round(close_c,3))
            return jsonify(close = close_update)
        return ""
